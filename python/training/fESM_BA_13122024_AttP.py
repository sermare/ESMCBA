...
run_id = f'fESM_BA_lr_{lr_input}_LTU_{layers_to_unfreeze}_ts_{size_of_train}_b_{batch_size}_r_{run}_loss_{loss}_atp'
...
class ESMFineTune(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None):
        ...
        att_scores = self.attention_weights(hidden_states).squeeze(-1) # (batch, seq_len)
        att_scores = att_scores.masked_fill(attention_mask == 0, float('-inf'))
        att_weights = torch.softmax(att_scores, dim=-1).unsqueeze(-1) # (batch, seq_len, 1)
    
        # Weighted sum
        pooled_output = (hidden_states * att_weights).sum(dim=1)
