...
run_id = f'numaug_{num_augmentations_input}_maskprob_{mask_prob_input}_lr_{lr_input}_{hla_type}_WL'

# Update the model_dir and loss_dir to include the run_id
strategy = 'fine-tune'  # or 'other strategy'

model_dir = f'/global/scratch/users/sergiomar10/models/all-HLA_{strategy}_{run_id}_WL/'
os.makedirs(model_dir, exist_ok=True)

loss_dir = f'/global/scratch/users/sergiomar10/losses/{run_id}_WL/'
...
# Gather all targets from the training dataset
all_targets = []
for _, _, _, targets_list in train_loader:  # Assuming train_loader is already prepared
    all_targets.extend([target for targets in targets_list for target in targets])
