...
parser.add_argument('--hla-type', type=str, default='HLA-A*02:01', help='Allele')
...
run_id = f'numaug_{num_augmentations_input}_maskprob_{mask_prob_input}_lr_{lr_input}_{hla_type}_LTU_{layers_to_unfreeze}_FL2'

# Update the model_dir and loss_dir to include the run_id
strategy = 'fine-tune'  # or 'other strategy'

model_dir = f'/global/scratch/users/sergiomar10/models/{strategy}_{run_id}/'
os.makedirs(model_dir, exist_ok=True)

loss_dir = f'/global/scratch/users/sergiomar10/losses/{run_id}/'
...
def generate_train_test_loader(dataset, hla_type):
    ...
    def tokenize_and_mask_sequences(sequences, num_augmentations=num_augmentations_input, mask_prob=mask_prob_input, max_length=15):
        ...
        for seq in sequences:
            ...
            # Identify positions corresponding to amino acids
            ...
            for _ in range(num_augmentations):
                if len(amino_acid_positions) >= num_masks:
                    ...
                    positions = amino_acid_positions  # Use all available positions
...
def masked_cross_entropy_loss(outputs, targets_list, masked_positions_list):
    ...
    # Compute the focal loss with class weights
...
# Generate train and evaluation loaders
...
# Collect all targets from the training dataset
all_targets = []
...
class FocalLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        pt = torch.sum(targets_one_hot * probs, dim=-1)  # p_t = probability of the true class
        focal_weight = (1 - pt) ** self.gamma

        ce_loss = -torch.sum(targets_one_hot * torch.log(probs + 1e-6), dim=-1)  # Cross-entropy loss
