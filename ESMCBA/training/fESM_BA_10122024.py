...
run_id = f'fESM_BA_lr_{lr_input}_LTU_{layers_to_unfreeze}_ts_{size_of_train}_b_{batch_size}_r_{run}_loss_{loss}_{HLA}'
...
model_dir = f'/global/scratch/users/sergiomar10/models/BA_MA/'
...
def prepare_data_for_hla(hla_type, batch_size=10, max_length=15,
    ...
    if sequences_and_BA is None:
        ...
        filtered_data = mhc_ligand_table[
            (mhc_ligand_table['MHC Restriction - Name'] == HLA) &
...
train_sequences, eval_sequences, eval_loader, sequences, test_labels = prepare_data_for_hla(HLA,
